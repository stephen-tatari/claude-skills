...
<!-- Based on claude-skill-codex by Yipeng Sun (https://github.com/sypsyp97/claude-skill-codex) -->
<!-- Original work licensed under MIT License. Copyright (c) 2025 Yipeng Sun. -->
...
| `--yolo` | flag | Bypasses all sandbox restrictions (alias for `--dangerously-bypass-approvals-and-sandbox`). **Cannot be used with --full-auto** |
...
| `--search` | flag | Enable live web search tool |
| `--enable/--disable` | `<FEATURE>` | Feature flag toggles |
...
1. Use `codex resume` to open the session picker
2. Try `--last` flag instead of specific ID
3. Check if session expired or was cleaned up
...
- Running with `--yolo` on machines with elevated privileges
